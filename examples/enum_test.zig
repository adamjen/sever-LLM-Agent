...
pub fn main() !void {
    ...
    {
        const _match_value = color;
        if (@intFromEnum(_match_value) == @intFromEnum(Color.Red)) {
            const _sever_main_result = 1;
            std_print_int(_sever_main_result);
            return;
        } else if (@intFromEnum(_match_value) == @intFromEnum(Color.Green)) {
            const _sever_main_result = 2;
            std_print_int(_sever_main_result);
            return;
        } else if (@intFromEnum(_match_value) == @intFromEnum(Color.Blue)) {
            const _sever_main_result = 3;
            std_print_int(_sever_main_result);
            return;
        }
    }
